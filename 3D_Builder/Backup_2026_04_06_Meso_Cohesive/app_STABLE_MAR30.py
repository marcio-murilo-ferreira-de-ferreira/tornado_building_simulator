...
with input_col:
    ...
    wind_direction = st.radio("Wind Direction", ["South Wall (Head-on)", "Global Out-of-plane"], horizontal=True)
...
with script_col:
    ...
    abq_col1, abq_col2, abq_col3, abq_col4 = st.columns(4)
    with abq_col1:
        def_scale_factor = st.slider("Def. Scale Factor", min_value=0.0, max_value=1000.0, value=1.0, step=1.0, help="1 is true scale.")
    with abq_col2:
    ...
    with abq_col3:
        mass_scaling = st.number_input("Target Time Step (Mass Scaling)", value=5e-5, step=1e-5, format="%.2e", help="Speeds up Explicit by scaling density to achieve this dt.")
    with abq_col4:
