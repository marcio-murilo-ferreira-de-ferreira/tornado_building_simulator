...
def generate_wall_preview(model_type, wall_width, wall_height, wall_thickness, 
    ...
    if "Macro-model" in model_type:
        ...
        if window_width and window_height:
            blocks = [
                dict(x=[0, wall_width], y=[0, w_yMin], z=[0, wall_thickness]), # Bottom
                dict(x=[0, wall_width], y=[w_yMax, wall_height], z=[0, wall_thickness]), # Top
                dict(x=[0, w_xMin], y=[w_yMin, w_yMax], z=[0, wall_thickness]), # Left
                dict(x=[w_xMax, wall_width], y=[w_yMin, w_yMax], z=[0, wall_thickness]) # Right
            ]
        else:
            blocks = [dict(x=[0, wall_width], y=[0, wall_height], z=[0, wall_thickness])]
        ...
        for w in range(wythes):
            z_start = w * (brick_depth + mortar_thickness)
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(title='X (Width) [m]', range=[0, wall_width], showbackground=False),
            yaxis=dict(title='Y (Height) [m]', range=[0, wall_height], showbackground=False),
            zaxis=dict(title='Z (Thickness) [m]', range=[-wall_thickness, wall_thickness*2], showbackground=False),
            aspectmode='data'
        ),
    ...
    if st.button(button_label, use_container_width=True, type="primary"):
        with st.spinner("Connecting to Roar Cluster..." if run_mode == "ROAR" else "Executing Abaqus CDP Analysis..."):
            ...
            if run_mode == "ROAR":
                ...
                success, msg = run_on_roar(psu_user, psu_pass, script_path, status_placeholder, log_container)
